...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const storedAdmin = sessionStorage.getItem('smartagro_admin');
    const storedAdminToken = sessionStorage.getItem('smartagro_admin_token');
    const storedUser = localStorage.getItem('smartagro_user');
    const storedToken = localStorage.getItem('smartagro_token');

    // Enforce single logged-in role at a time
    if (storedAdmin && storedAdminToken) {
      // Remove farmer session if admin session exists
      ...
      // Remove admin session if farmer session exists
      sessionStorage.removeItem('smartagro_admin');
      sessionStorage.removeItem('smartagro_admin_token');
  ...
  const login = async (email: string, password: string, role: 'farmer' | 'admin' = 'farmer'): Promise<boolean> => {
    ...
    try {
      ...
      if (role === 'admin' && data.token && data.admin) {
        // Clear farmer session if logging in as admin
        localStorage.removeItem('smartagro_user');
        localStorage.removeItem('smartagro_token');
        localStorage.removeItem('token');
        localStorage.removeItem('farmerId');
        
        sessionStorage.setItem('smartagro_admin', JSON.stringify(data.admin));
        sessionStorage.setItem('smartagro_admin_token', data.token);
        setUser({
          id: data.admin.id,
        ...
        // Clear admin session if logging in as farmer
        sessionStorage.removeItem('smartagro_admin');
        sessionStorage.removeItem('smartagro_admin_token');
        
        localStorage.setItem('smartagro_user', JSON.stringify(data.farmer));
