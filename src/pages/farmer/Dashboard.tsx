...
const Dashboard: React.FC = () => {
  ...
  const nepalLocations = [
    ...
    'Dharan', 'Bharatpur', 'Janakpur', 'Hetauda', 'Itahari', 'Nepalgunj', 
    'Butwal', 'Dhangadhi', 'Mahendranagar', 'Tikapur', 'Ghorahi', 'Tulsipur',
    'Damak', 'Dhankuta', 'Ilam', 'Jumla', 'Mugu', 'Humla', 'Dolpa', 'Mustang',
    'Manang', 'Gorkha', 'Lamjung', 'Kaski', 'Syangja', 'Tanahu', 'Nawalpur',
    'Chitwan', 'Makwanpur', 'Ramechhap', 'Dolakha', 'Sindhupalchok', 'Kavrepalanchok',
    'Sindhuli', 'Rasuwa', 'Nuwakot', 'Dhading', 'Gorkha', 'Palpa', 'Arghakhanchi',
    'Gulmi', 'Baglung', 'Myagdi', 'Parbat', 'Rukum', 'Rolpa', 'Pyuthan', 'Dang',
    'Salyan', 'Surkhet', 'Dailekh', 'Jajarkot', 'Kalikot', 'Mugu', 'Humla', 'Jumla',
    'Kailali', 'Kanchanpur', 'Baitadi', 'Darchula', 'Bajhang', 'Bajura', 'Achham',
    'Doti', 'Dadeldhura', 'Bardiya', 'Banke', 'Kapilvastu', 'Rupandehi', 'Nawalparasi',
    'Sarlahi', 'Mahottari', 'Dhanusha', 'Siraha', 'Saptari', 'Udayapur', 'Khotang',
    'Okhaldhunga', 'Solukhumbu', 'Sankhuwasabha', 'Terhathum', 'Panchthar', 'Ilam',
    'Jhapa', 'Morang', 'Sunsari', 'Dhankuta', 'Tehrathum', 'Panchthar'
  ];
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [subsidiesRes, issuesRes, marketRes, chatCountRes, diseaseScansRes] = await Promise.all([
          ...
          fetch('/api/market/market-prices'),
    ...
    const checkMarketUpdates = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const lastChecked = localStorage.getItem('last_market_check');
          
          if (lastUpdate && (!lastChecked || new Date(lastUpdate) > new Date(lastChecked))) {
            setNotifications(prev => [...prev, 'Market prices have been updated with the latest data!']);
            localStorage.setItem('last_market_check', lastUpdate);
          }
        }
      } catch (error) {
        console.error('Error checking market updates:', error);
      }
    };

    // Check immediately and then every 5 minutes
    checkMarketUpdates();
    const interval = setInterval(checkMarketUpdates, 5 * 60 * 1000);
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      try {
        // Use selected location for weather data
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-medium text-blue-900">System Updates</h3>
            ...
            <div className="border-t border-white border-opacity-20 p-6 animate-slideDown">
              ...
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Currently showing weather for: <strong>{selectedLocation}</strong>
                </p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <span className="text-xs text-gray-600">Quick select:</span>
                  {['Kathmandu', 'Pokhara', 'Biratnagar', 'Nepalgunj', 'Dharan'].map(location => (
                    <button
                      key={location}
                      onClick={() => {
                        setSelectedLocation(location);
