...
const CropDiseaseDetection: React.FC = () => {
  ...
  // Python FastAPI endpoint (proxied through Vite)
  const AI_API_URL = 'http://127.0.0.1:8000/api/predict';
  ...
  const fetchRecentScans = async () => {
    try {
      ...
      const response = await fetch('/api/disease/recent?limit=5', {
        headers: {
  ...
  try {
    ...
    // ✅ Correct FormData
    const formData = new FormData();
    formData.append('file', blob, 'crop-image.jpg'); // ✅ FIXED
    ...
    // ✅ FIX mapping (your backend returns: disease, confidence, remedies)
    ...
    const detectionResult = {
      ...
      severity: severity,
      treatment: data.remedies?.join(', ') || 'Consult expert',
      prevention: 'Follow proper crop care practices'
    };
    ...
    console.error(err);
    setError(`Could not detect disease: ${err.message}`);
  }
