...
# Get the absolute path to the trained model
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.expanduser("~/Downloads/AI_plant_diseases_detection (1).keras")
IMAGE_SIZE = 224

# Fallback model path in project
FALLBACK_MODEL_PATH = os.path.join(BASE_DIR, "model", "AI_plant_diseases_detection.keras")
...
def load_model():
    ...
    if TF_AVAILABLE and os.path.exists(MODEL_PATH):
        try:
            ...
            print(f"⚠️  Could not load Keras model: {e}. Using mock predictions.")
            return None
    else:
        print("⚠️  TensorFlow not available or model file not found. Using mock predictions.")
        return None
...
# -----
def load_model():
    """Load the trained VGG16 Keras model"""
    global model
...
# -----
def predict_disease(image_bytes: bytes, model=None):
    """Make prediction on plant disease using trained VGG16 model"""
    try:
...
# -----
# Predict Function (deprecated - use predict_disease instead)
# -----
def predict(image_bytes: bytes, model):
    ...
    if model is not None:
        try:
            ...
            print(f"Prediction error: {e}. Using mock prediction.")
            label = random.choice(class_names)
            confidence = random.uniform(75, 98)
    else:
